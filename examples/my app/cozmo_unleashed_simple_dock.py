...
import sys, os, datetime, random, time, math, re, threading,asyncio,cozmo
...
# CUBE USAGE
# activer ou non les cubes (économise la batterie si vous ne le faites pas).
# Je laisse presque toujours cette option désactivée, mais il continue à les empiler et à les manipuler.
# Certains jeux, comme "Block Guard Dog", ne s'affichent que si les blocs sont actifs.
#
...
# UTILISATION DES MURS DE MARQUAGE
# Si vous utilisez ou non des marqueurs imprimés pour les murs de votre terrain de jeu.
# Si vous utilisez des murs, vous devrez probablement configurer la taille des murs pour votre terrain de jeu.
#
...
def cozmo_unleashed(robot: cozmo.robot.Robot):
    ...
    #     robot.world.define_custom_wall(
    #         CustomObjectTypes.CustomType02, CustomObjectMarkers.Diamonds2, 785, 90, 40, 40, True)
    #     robot.world.define_custom_wall(
    #         CustomObjectTypes.CustomType03, CustomObjectMarkers.Diamonds3, 785, 90, 40, 40, True)
    #     robot.world.define_custom_wall(
    #         CustomObjectTypes.CustomType04, CustomObjectMarkers.Diamonds4, 530, 90, 40, 40, True)
    #     robot.world.define_custom_wall(
    #         CustomObjectTypes.CustomType05, CustomObjectMarkers.Diamonds5, 680, 90, 40, 40, True)
    ...
    if use_cubes == 2:
        # enable cube lights False to save cube's battery, True to turn cube lights on
...
# State 3: not on charger, good battery | default low battery is 3.65
        if (robot.battery_voltage > 3.7) and (robot.is_on_charger == 0):
            if freeplay == 0:
                ...
                print("State: turn on good time freeplay battery, battery %s" %
                      str(round(robot.battery_voltage, 2)))
				##
                if use_cubes >= 1:
...
# State 4: not on charger, low battery | default low battery is 3.65
        if (robot.battery_voltage <= 3.7) and (robot.is_on_charger == 0):
            if freeplay == 1:
                ...
                print("State: turn off good battery freeplay, battery %s" %
                      str(round(robot.battery_voltage, 2)))
				##
                robot.enable_all_reaction_triggers(False)
                robot.stop_all_motors()
                ...
                robot.play_anim_trigger(
                    cozmo.anim.Triggers.NeedsMildLowEnergyRequest, ignore_body_track=True).wait_for_completed()
                robot.wait_for_all_actions_completed()
                time.sleep(2)
				##
